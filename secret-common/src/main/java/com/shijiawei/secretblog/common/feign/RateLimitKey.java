...
public enum RateLimitKey {
    ...
    RATE_LIMIT_GLOBAL_COMMENT("ams:rate_limit:comment:global", "評論全局限流", RateType.OVERALL,20000, 1, RateIntervalUnit.MINUTES),

    // 用戶級別限流（按業務區分）
    RATE_LIMIT_USER_LIKE("ams:rate_limit:like:user:%s", "用戶點讚限流", RateType.OVERALL,30, 5, RateIntervalUnit.MINUTES),
    RATE_LIMIT_USER_COMMENT("ams:rate_limit:comment:user:%s", "用戶評論限流", RateType.OVERALL,10, 5, RateIntervalUnit.MINUTES),
