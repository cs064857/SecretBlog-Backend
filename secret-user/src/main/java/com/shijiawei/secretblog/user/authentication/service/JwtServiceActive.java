...
/**
 * JWT 服務 - 用於 Token 生成和驗證
 * 與 gateway 使用相同的密鑰對，確保統一認證
 */
@Service
public class JwtServiceActive implements InitializingBean {

    private static final Logger logger = LoggerFactory.getLogger(JwtServiceActive.class);
...
//            throw new CustomRuntimeException(ResultCode.JWT_CONFIG_ERROR.getCode(), ResultCode.JWT_CONFIG_ERROR.getMessage(),"獲取Jwt公鑰失敗", e.getCause());
    ...
    public static <T> T getPayload(String jwt, Class<T> jwtPayloadClass) {
        ...
        try {
            // jwt字符串由3部分組成，用英文的點分割：header.payload.sign
